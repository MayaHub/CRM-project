...
public class Employee extends Person implements Billable {
    ...
    //Employees receive 50% discount on walking services.
    @Override
    public double getBalanceDue(Map<String, Double> servicesRendered) {
        double amount = 0;
        double discountedWalkingRate = 27.5;

        for (Double value : servicesRendered.values()) {
            if (servicesRendered.containsKey("Walking")) {
                amount += discountedWalkingRate;
            } else {
                amount += value;
            }
        }
